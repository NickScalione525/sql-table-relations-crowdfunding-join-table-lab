...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT Projects.title, (SUM(Pledges.amount) - Projects.funding_goal) AS difference FROM projects INNER JOIN pledges ON projects.id = Pledges.project_id GROUP BY projects.title HAVING difference >= 0;"
end
